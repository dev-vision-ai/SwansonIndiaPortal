...
const METRIC_SPECS = { 
    ...
    'thicknessChart':   { label: 'Thickness', usl: 0.035, tgt: 0.030, lsl: 0.025, min: 0.020, max: 0.040, unit: 'mm', col: 'thickness', color: 'rgb(255, 99, 132)' }, 
    'opacityChart':     { label: 'Opacity', usl: 55.0, tgt: 50.0, lsl: 45.0, min: 40, max: 60, unit: '%', col: 'opacity', color: 'rgb(75, 192, 192)' }, 
    'cofChart':         { label: 'COF Kinetic', usl: 0.60, tgt: 0.40, lsl: 0.20, min: 0.10, max: 0.80, unit: 'COF', col: 'cof', color: 'rgb(255, 159, 64)' }, 
    // Page 2 
    ...
    'tensileStrength5MDChart': { label: 'Force~Tensile Strength@Break 5% (N)MD', usl: 5.5, tgt: 4.0, lsl: 2.5, min: 1.5, max: 6.5, unit: 'N', col: 'tensile-strength-5-md', table: 'testingTableBody2', color: 'rgb(76, 175, 80)' }, 
    // Page 3 
    'elongationCDChart': { label: 'Elongation@ Break (%) CD', tgt: 500, lsl: 400, min: 300, max: 650, unit: '%', col: 'elongation-cd', table: 'testingTableBody3', color: 'rgb(156, 39, 176)' }, 
    'tensileStrengthCDChart': { label: 'Force~Tensile Strength@Break (N) CD', tgt: 9.5, lsl: 6.5, min: 5.0, max: 12.0, unit: 'N', col: 'tensile-strength-cd', table: 'testingTableBody3', color: 'rgb(233, 30, 99)' }, 
    'modulusWebChart': { label: 'Modulus Web@ 2%', usl: 40.0, tgt: 30.0, lsl: 20.0, min: 10.0, max: 50.0, unit: 'N/cm', col: 'modulus-web', table: 'testingTableBody3', color: 'rgb(0, 150, 136)' }, 
    // Page 4 
    'colorLChart': { label: 'Color L', usl: 98.6, tgt: 94.6, lsl: 90.6, min: 85, max: 105, unit: 'H', col: 'color-l', table: 'testingTableBody4', color: 'rgb(121, 85, 72)' }, 
    'colorAChart': { label: 'Color A', usl: 2.9, tgt: -1.1, lsl: -5.1, min: -8, max: 5, unit: 'H', col: 'color-a', table: 'testingTableBody4', color: 'rgb(63, 81, 181)' }, 
    ...
    'colorDeltaEChart': { label: 'Color Delta E', usl: 5.00, tgt: 0.00, min: 0, max: 8, unit: 'Delta E', col: 'color-delta-e', table: 'testingTableBody4', color: 'rgb(255, 87, 34)' }, 
    'glossChart': { label: 'Gloss', usl: 11.0, tgt: 9.0, min: 5, max: 15, unit: 'Gloss unit', col: 'gloss', table: 'testingTableBody4', color: 'rgb(0, 0, 0)' } 
};
...
async function fetchInspectionDataFromDB() {
    try {
        ...
        const filmTableNames = [
            '168_18c_white_jeddah', // This page
            '168_16c_white',
            '168_16cp_kranti',
            '176_18cp_ww',
            '214_18_micro_white',
...
function getMetricData(spec, dbData = null) {
    ...
    // 2. Fallback to Table extraction (Primary for Page 2, 3, 4)
    const data = [];
    const sampleNumbers = [];
    ...
    for (const row of rows) {
        ...
        if (tableBodyId === 'testingTableBody') {
            const map = { 'basis-weight': 3, 'thickness': 4, 'opacity': 5, 'cof': 6 };
            ...
            const map = { 'color-l': [3], 'color-a': [4], 'color-b': [5], 'color-delta-e': [6], 'gloss': [7,8,9] };
            columnIndices = map[columnType] || [];
        }
        ...
        if (validValues.length > 0) {
            ...
            // Page 1 specific: only add positive values
...
window.ChartRenderer = {
    ...
    createInspectionResults: createInspectionResultsChart,
    createDefectRate: createDefectRateChart,
    createThickness: createThicknessChart,
