...
function createChart(canvasId, chartType, data = null, customOptions = {}) {
        ...
        options: {
            ...
            plugins: {
                legend: {
                    position: 'top',
                    display: true,
                    labels: { filter: (legendItem) => legendItem.text !== null && legendItem.text !== undefined && legendItem.text !== '' }
                },
                tooltip: {
                    enabled: true,
                    mode: 'index',
                    ...
                    position: 'nearest',
                    yAlign: 'top',
                    xAlign: 'center',                    displayColors: false,
                    caretSize: 6,
                    caretPadding: 6,
                    ...
                    borderWidth: 1,
                    titleColor: '#fff',
                    bodyColor: '#fff',                    filter: (tooltipItem) => tooltipItem.dataset.label !== null && tooltipItem.dataset.label !== undefined,
                    callbacks: {
    ...
    if (customOptions) {
        config.options = {
            ...config.options,
            ...customOptions,
            plugins: { ...config.options.plugins, ...(customOptions.plugins || {}) },
            scales: { ...config.options.scales, ...(customOptions.scales || {}) }
        };
    }
        ...
        const chart = createChart(id, 'line', { labels, datasets }, {
            ...
            scales: {
                y: {
                    ...
                    ticks: { callback: function(value) { return formatChartValue(value, id); } }
                }
            }
        });
        if (!chart) {
...
function getMetricData(spec, dbData = null) {
    ...
    for (const row of rows) {
        ...
        if (tableBodyId === 'testingTableBody') {
            ...
            // Cell 0-2: Sample No (colspan=3) | Cell 3: Basis Weight | Cell 4: COF-RR | Cell 5: COF-RS | Cell 6: Opacity | Cell 7-10: Modulus 1,2,3,Avg | Cell 11: Gloss
            const map = { 
                'basis-weight': [3], 
                'opacity': [6], 
                'cof-rr': [4], 
            ...
            // Page 2: Sample No (0-2 actual cells, but colspan=3) + Elong MD (3) + Tensile MD (4) + Elong CD (5) + Tensile CD (6) + Color L (7) + Color A (8) + Color B (9) + DeltaE (10)
            // BUT with colspan cells, the actual cell count is: 1 (sample) + 1 (elong md) + 1 (tensile md) + 1 (elong cd) + 1 (tensile cd) + 1 (color l) + 1 (color a) + 1 (color b) + 1 (delta e) = 9 cells
            // However the color cells have colspan=3 for the dash, so we need to figure out actual indices
            const map = { 
                'tensile-strength-5-md': [1],      // 2nd cell (after sample col with colspan)
                'tensile-strength-md': [2],        // 3rd cell
                'tensile-strength-5-cd': [3],      // 4th cell
                'tensile-strength-cd': [4],        // 5th cell
                'color-l': [5],                    // 6th cell
                'color-a': [6],                    // 7th cell
                'color-b': [7],                    // 8th cell
                'color-delta-e': [8]               // 9th cell
            };
