...
const CONFIG = {
    ...
    // Table Configuration
    SUMMARY_ROWS_COUNT: 3, // Number of summary rows (Average, Min, Max)
    ...
    // UI Timing Delays
    UI_DELAY: 100,         // ms for UI interactions
    LOAD_DELAY: 500,       // ms for data loading operations

    // Equipment Dropdown IDs
    EQUIPMENT_IDS: {
        // Page 1 Equipment
    ...
    EQUIPMENT_MAPPINGS: {
        ...
        'Spectrophotometer': ['OPACITY'],
        'Instron': ['COF', 'PAGE2_COMMON', 'PAGE3_COMMON'],
        'Steel Ruler': ['CUT_WIDTH'],
        'X-RITE': ['COLOR_UNPRINTED', 'COLOR_PRINTED'],
        'Glossmeter': ['GLOSS']
    },
...
// ===== HELPER FUNCTIONS =====
// Centralized helper functions for repetitive DOM operations
const DOM_HELPERS = {
    ...
    // Batch operations for all equipment dropdowns
    setAllEquipmentLoading: function () {
        Object.values(CONFIG.EQUIPMENT_IDS).forEach(id => this.setDropdownLoading(id));
    },

    setAllEquipmentError: function () {
...
const SPEC_CONFIG = {
    ...
    basicWeight: { min: 14, max: 18 },
    thickness: { min: 0.025, max: 0.035 },
    opacity: { min: 45.0, max: 55.0 },
    cof: { min: 0.20, max: 0.60 },
    cutWidth: { min: 166, max: 170 },
    colorDelta: { max: 4.00 }, // Only upper limit

    // Page 2 Specifications  
    elongationMD: { min: 350 }, // Only lower limit
    forceMD: { min: 9.0 }, // Only lower limit
    force5pMD: { min: 2.5, max: 5.5 },

    // Page 3 Specifications
    elongationCD: { min: 400 }, // Only lower limit
    forceCD: { min: 6.0 }, // Only lower limit
    modulus: { min: 20.0, max: 40.0 },

    // Page 4 Specifications
    gloss: { max: 11.0 }, // Only upper limit
    pgQuality: { validValues: [0, 1] } // Only 0 or 1 are valid
};
...
function formatDateToDDMMYYYY(dateString) {
    ...
    const day = String(date.getDate()).padStart(CONFIG.DATE_FORMATTING.PAD_LENGTH, CONFIG.DATE_FORMATTING.PAD_ZERO);
    const month = String(date.getMonth() + 1).padStart(CONFIG.DATE_FORMATTING.PAD_LENGTH, CONFIG.DATE_FORMATTING.PAD_ZERO);
...
function initializeVerification() {
    ...
    const passwordInput = document.getElementById(CONFIG.DOM_IDS.VERIFICATION_PASSWORD);
    ...
    if (verifyBtn) {
        verifyBtn.addEventListener('click', function () {
            ...
            if (!verificationDate) {
                ...
                DOM_HELPERS.focusInput(CONFIG.DOM_IDS.VERIFICATION_DATE);
                return;
            }
            ...
            if (enteredPassword === VERIFICATION_PASSWORD) {
                ...
                DOM_HELPERS.clearInput(CONFIG.DOM_IDS.VERIFICATION_PASSWORD, true);
            }
        });
    }
    ...
    if (cancelBtn) {
        cancelBtn.addEventListener('click', function () {
            DOM_HELPERS.clearInput(CONFIG.DOM_IDS.VERIFICATION_PASSWORD);
...
function showApprovalStatus() {
    ...
    // Populate approval status display
    const formId = getCurrentFormId();
    if (!formId) return;

    supabase
        .from('168_16cp_kranti')
        .select('approved_by, approved_date')
        .eq('form_id', formId)
        .maybeSingle()
        .then(({ data, error }) => {
...
function setApprovalFormState(isEnabled) {
    const approvalPassword = document.getElementById(CONFIG.DOM_IDS.APPROVAL_PASSWORD);
    ...
    if (approvalPassword) {
        ...
        approvalPassword.readOnly = !isEnabled;
    }
    if (approvalDate) {
        approvalDate.disabled = !isEnabled;
        approvalDate.readOnly = !isEnabled;
    }
    if (approveFormBtn) approveFormBtn.disabled = !isEnabled;
    if (cancelApprovalBtn) cancelApprovalBtn.disabled = !isEnabled;
}
...
function initializeApproval() {
    const approvalForm = document.getElementById('approvalForm');
    const approvalStatus = document.getElementById('approvalStatus');
    const approvalPasswordInput = document.getElementById(CONFIG.DOM_IDS.APPROVAL_PASSWORD);
    ...
    // Wait a bit for form_id to be available, then check approval status
    setTimeout(() => {
        checkApprovalStatus();
    }, 500);
    ...
    if (approveBtn) {
        approveBtn.addEventListener('click', function () {
            const enteredPassword = approvalPasswordInput.value.trim();
            const approvalDate = approvalDateInput.value;

            if (!approvalDate) {
                alert('Please select an approval date.');
                DOM_HELPERS.focusInput(CONFIG.DOM_IDS.APPROVAL_DATE);
                return;
            }

            if (enteredPassword === APPROVAL_PASSWORD) {
                approveForm(approvalDate);
            } else {
                alert('Invalid approval password. Please contact QA.');
                DOM_HELPERS.clearInput(CONFIG.DOM_IDS.APPROVAL_PASSWORD, true);
            }
        });
    }
    ...
    if (cancelBtn) {
        cancelBtn.addEventListener('click', function () {
            DOM_HELPERS.clearInput(CONFIG.DOM_IDS.APPROVAL_PASSWORD);
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function showEquipmentLoadingState() {
        DOM_HELPERS.setAllEquipmentLoading();
    }
    ...
    function populateEquipmentDropdowns(equipmentData) {
        ...
        const equipmentMappings = CONFIG.EQUIPMENT_MAPPINGS;

        // Populate dropdowns
        Object.keys(equipmentMappings).forEach(equipmentType => {
            const configKeys = equipmentMappings[equipmentType];
            const equipmentIds = equipmentByType[equipmentType] || [];

            configKeys.forEach(key => {
                const dropdownId = CONFIG.EQUIPMENT_IDS[key];
                const dropdown = document.getElementById(dropdownId);
                if (dropdown) {
    ...
    function showEquipmentLoadingError() {
        DOM_HELPERS.setAllEquipmentError();
    }
    ...
    function validateInput(input, fieldType) {
        ...
        // Handle maxLength validation (for text fields like LOT_ROLL, ROLL_ID, etc.)
        if (pattern.maxLength !== undefined && value.length > pattern.maxLength) {
            value = value.substring(0, pattern.maxLength);
        }

        // Handle decimal validation (for numeric fields)
        if (pattern.beforeDecimal !== undefined || pattern.afterDecimal !== undefined) {
            if (parts.length === 2) {
                ...
                // No decimal point yet, limit to beforeDecimal digits
                if (pattern.beforeDecimal !== undefined && parts[0].length > pattern.beforeDecimal) {
                    parts[0] = parts[0].substring(0, pattern.beforeDecimal);
                    value = parts[0];
                }
    ...
    let saveTimeout = null; // Timer for debounced saves
    function debouncedSave() {
        clearTimeout(saveTimeout);
        saveTimeout = setTimeout(autoSaveToDatabase, CONFIG.DEBOUNCE_DELAY); // delay for better responsiveness
    }
    ...
    // Removed unused clearDataRows function

    // Show notification
    function showNotification(message, type = 'info') {
        // Create notification element
        const notification = document.createElement('div');
        notification.className = `fixed top-20 right-4 p-4 rounded-lg shadow-lg z-50 max-w-sm ${type === 'success' ? 'bg-green-500 text-white' :
    ...
    function handleKeyboardNavigation(e, currentCell) {
        ...
        switch (e.key) {
            ...
            case 'Enter':
                e.preventDefault();
                // Custom navigation for specific pages and columns
                ...
                if (isSpecialTable) {
                    ...
                    // Gloss 1/2/3 for Page 4 (Columns 3, 4, 5)
                    if (cellIndex >= 3 && cellIndex <= 5) {
                        isSpecialColumn = true;
                        groupStart = 3;
                        groupEnd = 5;
    ...
    function addRowsToTable(tableBody, n) {
        ...
        if (existingDataRows.length > 0) {
            ...
            if (lotCell && lotCell.textContent.trim()) {
                ...
                if (!isNaN(lastLotNum)) {
                    lotNumber = String(lastLotNum + 1).padStart(CONFIG.DATE_FORMATTING.PAD_LENGTH, CONFIG.DATE_FORMATTING.PAD_ZERO);
                }
            }
        }
    ...
    function applyValidationToInput(input, tableBody, columnIndex) {
        ...
        if (tableBody.id === 'testingTableBody' && columnIndex === 3) {
            input.addEventListener('input', function () {
                validateInput(this, 'BASIC_WEIGHT');
            ...
            input.addEventListener('keydown', function (e) {
                if (e.key === 'Enter') {
                    ...
                    if (nextRow && !['Average', 'Minimum', 'Maximum'].includes(nextRow.querySelector('td').textContent.trim())) {
                        DOM_HELPERS.focusElement(nextRow.querySelector('input'));
                    }
                }
            });
    ...
    function formatLotRollOnEnter(input) {
        ...
        if (parts.length === 2) {
            ...
            const part1 = parts[0].padStart(CONFIG.DATE_FORMATTING.PAD_LENGTH, CONFIG.DATE_FORMATTING.PAD_ZERO);
            const part2 = parts[1].padStart(CONFIG.DATE_FORMATTING.PAD_LENGTH, CONFIG.DATE_FORMATTING.PAD_ZERO);
    ...
    function formatLotTimeOnEnter(input) {
        ...
        if (parts.length === 2) {
            // Format both parts to 2 digits with leading zeros
            const hours = parts[0].padStart(CONFIG.DATE_FORMATTING.PAD_LENGTH, CONFIG.DATE_FORMATTING.PAD_ZERO);
            const minutes = parts[1].padStart(CONFIG.DATE_FORMATTING.PAD_LENGTH, CONFIG.DATE_FORMATTING.PAD_ZERO);
    ...
    function applyColorFormatting(input, columnType) {
        ...
        const spec = SPEC_CONFIG[columnType];
        if (spec) {
            const valueNum = parseFloat(value);
            if (!isNaN(valueNum)) {
                // Check for out-of-spec conditions
                if (spec.validValues) {
                    shouldHighlight = !spec.validValues.includes(valueNum);
                } else {
                    shouldHighlight = (spec.min !== undefined && valueNum < spec.min) ||
                        (spec.max !== undefined && valueNum > spec.max);
                }
            }
        }
