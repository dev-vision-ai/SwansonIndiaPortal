...
function setFormTitle() {
    ...
    if (formTitle) {
        ...
        if (productCodeInput && productCodeInput.value) {
            ...
            formTitle.textContent = 'UC-16gsm-165W';
        }
    }
}
...
// Historical data logic removed per user request
...
function getTableDataFromAllTables() {
    ...
    allTables.forEach(tableBody => {
        ...
        if (tableId === 'testingTableBody') {
            ...
            // Page 1 data - Updated to match actual HTML structure (6 columns total)
            ...
            tableData.page1_cof_rr = convertColumnToJSONB(tableBody, 5); // COF (R-R)
            tableData.page1_tensile_break = convertColumnToJSONB(tableBody, 6); // Tensile Break
            tableData.page1_elongation = convertColumnToJSONB(tableBody, 7); // MD Elongation Break
            tableData.page1_modulus = convertColumnToJSONB(tableBody, 8); // 10% Modulus
            ...
            // Page 3 data - 4 columns (Color Measurements)
...
function applyOOSValidation(input, columnType) {
    ...
    switch(columnType) {
        // Page 1 parameters
        case 'filmWeight':
            // Film Weight: L-15.00 T-16.00 U-18.00 g/m2
            shouldHighlight = value < 15.00 || value > 18.00;
            break;
        case 'thickness':
            // Thickness: L-32 T-42 U-52 μm
            shouldHighlight = value < 32 || value > 52;
            break;
        case 'cofRR':
        ...
        case 'modulus10':
            ...
            break;
        case 'opacity':
            // Opacity: L-42.0 T-48.0 U-54.0 (full range)
            shouldHighlight = value < 42.0 || value > 54.0;
            break;
        case 'rollWidth':
            // Roll Cut Width: T-165 U-168 mm (target and upper limit)
            shouldHighlight = value < 165 || value > 168;
            break;
        case 'diameter':
            // Diameter: L-430 T-460 U-510 mm (full range)
            shouldHighlight = value < 430 || value > 510;
            break;
        // Page 3 parameters
        case 'colourL':
            // Colour L: L-90.6 T-94.6 U-98.6 (Colour units-H)
            shouldHighlight = value < 90.6 || value > 98.6;
            break;
        case 'colourA':
            // Colour A: L-(-5.1) T-(-1.1) U-2.9 (Colour units-H)
            shouldHighlight = value < -5.1 || value > 2.9;
            break;
        case 'colourB':
            // Colour B: L-(-3.6) T-0.4 U-4.4 (Colour units-H)
            shouldHighlight = value < -3.6 || value > 4.4;
            break;
        case 'deltaE':
            // Delta E: T-0.0 U-5.0 (Colour Units-Delta E) - only upper limit
            shouldHighlight = value > 5.0;
            break;
    }
...
function calculateSummaryStatistics(tableBody) {
    ...
    // Page 1 columns: Film Weight (3), Thickness (4), COF-RR (5), Tensile Break (6), MD Elongation Break (7), 10% Modulus (8)
    const summaryColumnIndices = [1, 2, 3, 4, 5, 6]; // Summary row column positions (6 data columns)
    const inputColumnIndices = [3, 4, 5, 6, 7, 8]; // Input columns in data rows (after Sample No colspan=3) - 6 columns
...
function updateTabOrderForAllRows(tableBody) {
    ...
    let columnCount;
    if (tableBody.id === 'testingTableBody') {
        columnCount = 9; // Page 1: 3 Sample No + 6 data columns (Film Weight, Thickness, COF-RR, Tensile Break, MD Elongation, 10% Modulus) = 9 columns
        ...
        columnCount = 7; // Page 3: 3 Sample No + 4 data columns (Colour L, Colour A, Colour B, Delta E) = 7 columns
...
// Function to add rows to any table
    ...
    if (isPage1) {
        ...
        columnCount = 9; // Default fallback
           }
        ...
        tr.className = 'border border-gray-800 px-3 py-2 text-center';
            ...
            if (j === 0) {
                ...
                if (tableBody.id === 'testingTableBody') {
                    ...
                    // Page 3: First column should be read-only (synced from Page 1)
                    input.readOnly = true;
                    input.style.backgroundColor = 'transparent';
                    input.style.color = '#000000';
                ...
                if (tableBody.id === 'testingTableBody') {
                    ...
                    // Page 3: Second column should be read-only (synced from Page 1)
                    input.readOnly = true;
                    input.style.backgroundColor = 'transparent';
                    input.style.color = '#000000';
                    input.style.textAlign = 'center';
                ...
                if (tableBody.id === 'testingTableBody') {
                    ...
                    // Page 3: Third column should be read-only (synced from Page 1)
                    input.readOnly = true;
                    input.style.backgroundColor = 'transparent';
                    input.style.color = '#000000';
                    input.style.textAlign = 'center';
                ...
                if (isPage1) {
                    // Page 1 validation (6 data columns: 3-8)
                    if (j === 3) {
                        ...
                        // MD Elongation Break column - 3 digit format (000)
                        applyThreeDigitValidation(input);
                        // Apply OOS validation
                    ...
                    // Page 3 validation (4 columns only)
                    if (j === 3) {
    ...
    try {
        ...
        if (tableBody.id === 'testingTableBody') {
            ...
            if (data.page1_cof_rr) {
                loadColumnDataToTable(tableBody, 5, data.page1_cof_rr);
            }
            if (data.page1_tensile_break) {
                loadColumnDataToTable(tableBody, 6, data.page1_tensile_break);
            }
            if (data.page1_elongation) {
                loadColumnDataToTable(tableBody, 7, data.page1_elongation);
            }
            if (data.page1_modulus) {
                loadColumnDataToTable(tableBody, 8, data.page1_modulus);
            }
...
function loadTableDataFromDatabase(data) {
    ...
    if (data.page1_cof_rr) {
        loadColumnDataToTable(testingTableBody, 5, data.page1_cof_rr);
    }
    
    if (data.page1_tensile_break) {
        loadColumnDataToTable(testingTableBody, 6, data.page1_tensile_break);
    }

    if (data.page1_elongation) {
        loadColumnDataToTable(testingTableBody, 7, data.page1_elongation);
    }

    if (data.page1_modulus) {
        loadColumnDataToTable(testingTableBody, 8, data.page1_modulus);
    }
        ...
        if (rowIndex >= 0 && rowIndex < dataRows.length) {
            ...
            if (input) {
                ...
                if (inputIndex > 2) {
                    ...
                    if (tableBody.id === 'testingTableBody') {
                        // Page 1 columns (6 columns only)
                        if (inputIndex === 3) columnType = 'filmWeight';
                        else if (inputIndex === 4) columnType = 'thickness';
                        else if (inputIndex === 5) columnType = 'cofRR';
                        else if (inputIndex === 6) columnType = 'tensileBreak';
                        else if (inputIndex === 7) columnType = 'elongation';
                        else if (inputIndex === 8) columnType = 'modulus10';
                    } else if (tableBody.id === 'testingTableBody2') {
                        // Page 2 columns
...
// Historical data functions removed per user request
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // Historical data setup removed per user request
    ...
    if (addRowsBtn) {
        addRowsBtn.addEventListener('click', async function() {
            ...
            if (rowsToAdd > 0) {
                ...
                // Add rows to all tables to reach 30 total (Pages 1, 2 & 3 for UC-16gsm-165W)
            ...
            // Historical data loading removed per user request
