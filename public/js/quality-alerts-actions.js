...
document.getElementById('sendAlertButton').addEventListener('click', async function() {
  ...
  const alertLink = `${window.location.origin}${basePath}/html/quality-alerts-actions.html?id=${alertId}&action=edit`;
...
deleteButton.addEventListener('click', async function () {
    ...
    if (confirm('Are you sure you want to delete this alert?')) {
        try {
            ...
            setTimeout(() => {
                window.history.replaceState({}, '', 'quality-alerts-table.html');
                window.location.href = 'quality-alerts-table.html';
            }, 300);
        } catch (error) {
